...
class StoragesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create storage' do
    assert_difference('Storage.count') do
      post storages_url,
           params: { storage: { locations: @storage.locations, storage_price: @storage.storage_price,
                                storage_staff: @storage.storage_staff, storage_type: @storage.storage_type, user_id: @storage.user_id } }
    end
